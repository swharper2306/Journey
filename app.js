...
(() => {
  ...
  function setupStickerRoom() {
    ...
    // IMPORTANT: grab the room element BEFORE we use it (prevents
    // "Cannot access 'room' before initialization" errors).
    const room = $("#room");
    if (!room) return;
    ...
    function setRoomBg(key) {
      const k = key || "look1";
      // Layer a gentle overlay on top of the photo so stickers/text stay readable.
      room.style.backgroundImage =
        `linear-gradient(180deg, rgba(0,0,0,.10), rgba(0,0,0,.28)), url(./assets_journey/images_journey/${k}.jpg)`;
